...
let storeTable = document.getElementById('stores');
...
const totalCookieFooter = () => {
  let tfootEl = document.createElement('tfoot'); // created a tfoot in my html doc
  console.log(tfootEl);
  let trEl = document.createElement('tr'); // created a <tr> in my html doc
  let tdEl = document.createElement('td');  // created a <td> in my html doc
  console.log(tdEl);
...
const Store = (nameOfStore, minCustomers, maxCustomers, avgCookiesSold) => {
  ...
  this.cookiesSoldPerEachHour = () => {
    for (let cookieCounter = 0; cookieCounter < hoursOfOperation.length; cookieCounter++) {

      let avgCookiesPurchased = Math.floor(this.numOfCustomers() * this.avgCookiesSold);
...
Store.prototype.render = () => {
  ...
  for (let i = 0; i < this.salesPerEachHour.length; i++) {
...
const addNewStore = (event) => {
  ...
  let newMinCustomers = event.target.minCustomers.value;
  let newMaxCustomers = event.target.maxCustomers.value;
  let newAvgCookiesSold = event.target.avgCookiesSold.value;
...
let firstAndPike = new Store('First and Pike', 23, 65, 6.3);
